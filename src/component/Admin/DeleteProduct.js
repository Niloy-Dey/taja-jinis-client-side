import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import useProduct from '../../hooks/useProduct';

const DeleteProduct = () => {


    const [products, setProducts] = useProduct([]);
    // const [outStandingData, setoutStandingData] = useState([]);


    const handleDelete = id => {
        console.log(id);
        const proceed = window.confirm('Are you sure delete this item ?');
        if (proceed) {
            // const url = `http://localhost:5000/products/${id}`;
            const url = `https://taja-jinis-backend.onrender.com/products/${id}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
                    console.log(data);
                    const remaining = products.filter(order => order._id !== id);
                    setProducts(remaining);

                })

        }
    }

    return (
        <div>
            <div className='text-center mt-20 flex justify-center items-center'>
                <div>
                    <h1 className='text-orange-400 text-2xl font-bold  mb-3 ms-5 ps-3'>পণ্য মুছে ফেলুন</h1>
                    <div className='w-50 mx-auto ps-5 mt-5 pt-5 grid lg:grid-cols-3 md:grid-cols-2  grid-cols-1 gap-5 '>


                        {
                            products.map(tool =>
                                <div className='card-style my-5 mx-5  ' key={tool.id}>
                                    <div className='flex justify-around	 w-90 h-40 bg-green-100 shadow-lg rounded-lg p-10 '>
                                        <img className='w-20 mr-2' src={tool.image} alt="" />
                                        <div className='mx-2'>
                                            <h5> <b>{tool.name}</b> </h5>
                                            <h5> <b>{tool.price}</b> </h5>
                                            <h5> <b>{tool.quantity}</b> </h5>
                                        </div>
                                        <h5 className='ml-2'><button onClick={() => handleDelete(tool._id)} className="delete-button text-black bg-red-300 hover:bg-gray-600  rounded-lg hover:text-white btn-sm text-xs"> মুছে ফেলুন  </button></h5></div>
                                </div>)
                        }

                    </div>
                </div>
            </div>
            <div className='text-center'>
                <Link to='/admin/addProduct'> <button className="btn bg-green-400">নতুন পণ্য যোগ করুন</button> </Link>
            </div>
        </div>
    );
};

export default DeleteProduct;